...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        std::cerr << "错误：必须指定complex.in2文件路径\n"
            << "用法：" << argv[0]
            << " in2_file [tape_file]\n"
            << "（后面两个参数已强制使用默认值）" << std::endl;
        return 1;
    }

    // 获取exe目录并设置默认路径
    std::string exeDir = getExeDirectory();
    std::string defaultTape = resolvePath(exeDir, "tapeFileNew.txt");
    std::string defaultPoints = resolvePath(exeDir, "pointCoord.txt");
    std::string defaultAstro = resolvePath(exeDir, "astroPoints.txt");
    ...
    std::string in2Path = resolvePath(exeDir, argv[1]);
    std::string tapePath;
    bool processTape = true;

    // 处理第二个参数
    if (argc >= 3) {
        tapePath = resolvePath(exeDir, argv[2]);
        if (!fileExists(tapePath)) {
            std::cerr << "错误：无法找到标尺文件 " << tapePath << std::endl;
            return 1;
        }
    }
    else {
        tapePath = defaultTape;
        if (!fileExists(tapePath)) {
            processTape = false;
        }
    }
    ...
    // 固定使用默认值的参数
    std::string pointsPath = defaultPoints;
    std::string astroPath = defaultAstro;
    ...
    if (processTape) {
        ...
        std::cout << "FROM\tTO\tFW_VALUE\tV\tRESULT" << std::endl;
        for (const auto& res : V_results) {
            std::cout << res.FROM << '\t' << res.TO << '\t' << res.VALUE << '\t'
                << res.V << '\t' << res.RESULT << std::endl;
        }
